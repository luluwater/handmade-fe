...
const Reply = ({ id, account, createTime, user_id, reply, avatar }) => {
  const [deleteReply] = useDeleteReplyMutation()
  const userData = JSON.parse(localStorage.getItem('user'))?.user
  const isCurrentUser = user_id === userData?.id
  ...
  return (
    <>
      <div
        ...
        className={`w-100 w-md-50 ms-auto mb-4 ${
          isCurrentUser ? 'bg-white' : 'bg-skin-bright'
        } p-3 rounded-3`}
      >
